...
# Active build: v0.5.36 CONT GUARD.
...
# - DBG SET / DBG REV / DBG CHOP / DBG CONT / DBG BOTH / DBG ESC / DBG MIX / DBG PROBE labels separate setup, review, chop block, continuation block, detected conflict, escaped conflict, blocked conflict, and probe cadence.
...
def reviewLongConflictEscape = fastBreakoutConfirm or (reviewConflictEfficientMove and reviewLongConflictScoreDominant and close > emaSlow and close > vwapLine);
def reviewShortConflictEscape = fastBreakdownConfirm or (reviewConflictEfficientMove and reviewShortConflictScoreDominant and close < emaSlow and close < vwapLine);
...
plot ReviewLongArrowV0536 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0536 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0536.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0536.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0536.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0536.AssignValueColor(Color.CYAN);
ReviewLongArrowV0536.SetLineWeight(5);
ReviewShortArrowV0536.SetLineWeight(5);

plot ScoreProbeLongArrowV0536 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0536 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0536.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0536.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0536.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0536.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0536.SetLineWeight(3);
ScoreProbeShortArrowV0536.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.36 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.36 CONT GUARD", Color.BLACK);
