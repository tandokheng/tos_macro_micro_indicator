...
def longEntryConfirm = longCandleConfirm or longBreakConfirm;
def shortEntryConfirm = shortCandleConfirm or shortBreakConfirm;
...
def longScoreFresh = BarsSinceTrue(longScoreCrossEvent) <= scoreTriggerFreshBars;
def shortScoreFresh = BarsSinceTrue(shortScoreCrossEvent) <= scoreTriggerFreshBars;
...
def longCautionSetup = allowCautionFourOfSix and longScore == standbyThreshold and longScore > shortScore and longTradeOK and longEntryConfirm;
def shortCautionSetup = allowCautionFourOfSix and shortScore == standbyThreshold and shortScore > longScore and shortTradeOK and shortEntryConfirm;
...
def barsSincePlottedEntry =
    CompoundValue(
        1,
        if newLongEntry[1] or newShortEntry[1] then 1 else barsSincePlottedEntry[1] + 1,
        cooldownBars + 1
    );

def cooldownOK = barsSincePlottedEntry > cooldownBars;
...
def trendResetLong = lastEntryDir[1] == 1 and (shortBias or longScore < triggerThreshold);
def trendResetShort = lastEntryDir[1] == -1 and (longBias or shortScore < triggerThreshold);
def sameSideLongExpired = lastEntryDir[1] == 1 and barsSincePlottedEntry > resetSameSideAfterBars and !trendResetLong;
def sameSideShortExpired = lastEntryDir[1] == -1 and barsSincePlottedEntry > resetSameSideAfterBars and !trendResetShort;
def sameSideLongRefresh = sameSideLongExpired and !sameSideLongExpired[1];
def sameSideShortRefresh = sameSideShortExpired and !sameSideShortExpired[1];

def longEntryPulse =
    (rawLongEntry or longReadyEntry) and
    (longEntryEdge or longReadyEdge or cooldownJustCleared or longTradeJustUnblocked or sameSideLongRefresh);
def shortEntryPulse =
    (rawShortEntry or shortReadyEntry) and
    (shortEntryEdge or shortReadyEdge or cooldownJustCleared or shortTradeJustUnblocked or sameSideShortRefresh);

def lastEntryDir =
    CompoundValue(
        1,
        if trendResetLong then 0
        else if trendResetShort then 0
        else if newLongEntry then 1
        else if newShortEntry then -1
        else lastEntryDir[1],
        0
    );

def newLongEntry = longEntryPulse and cooldownOK and (lastEntryDir[1] != 1 or sameSideLongExpired);
def newShortEntry = shortEntryPulse and cooldownOK and (lastEntryDir[1] != -1 or sameSideShortExpired);
def longSignalCaution = longScore < triggerThreshold or longTradeCaution;
def shortSignalCaution = shortScore < triggerThreshold or shortTradeCaution;
...
AddLabel(
    ...
    "WHY: " +
        (if dashDir == 1 then
            ...
            (if dashScore < standbyThreshold or (dashScore < triggerThreshold and !allowCautionFourOfSix) then "SCORE"
             else if !shortTradeOK then (if structureBlocked then "STRUCT" else "RVOL")
