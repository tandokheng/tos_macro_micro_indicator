...
def visualLongSetupReady = no;
def visualShortSetupReady = no;
...
def reviewLongSignal = realLongEntry;
def reviewShortSignal = realShortEntry;
...
def dashboardTriggerLong = fastFlipLongArrow or dashboardContLong or realLongEntry;
def dashboardTriggerShort = fastFlipShortArrow or dashboardContShort or realShortEntry;
...
def longSignalCaution = longScore < triggerThreshold or longTradeCaution or fastFlipLongArrow or longContinuationArrow;
def shortSignalCaution = shortScore < triggerThreshold or shortTradeCaution or fastFlipShortArrow or shortContinuationArrow;
...
plot ReviewLongArrowV0521 =
    if showReviewSignalArrows and reviewLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0521 =
    if showReviewSignalArrows and reviewShortSignal then high + off else Double.NaN;
ReviewLongArrowV0521.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0521.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0521.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0521.AssignValueColor(Color.CYAN);
ReviewLongArrowV0521.SetLineWeight(5);
ReviewShortArrowV0521.SetLineWeight(5);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.21 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.21 QUIET ARROWS", Color.BLACK);
