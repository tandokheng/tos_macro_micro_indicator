...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.13 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.13 SPAM DIAG", Color.BLACK);
AddLabel(yes, "MARKER: " + (if arrowMarkerLong and arrowMarkerShort then "BOTH" else if arrowMarkerLong then "L" else if arrowMarkerShort then "S" else "NONE"), if markerContractFail then Color.RED else if arrowMarkerLong then Color.MAGENTA else if arrowMarkerShort then Color.CYAN else Color.GRAY);
AddLabel(yes, "CONTRACT: " + (if markerContractFail then "FAIL TRIGGER/MARKER" else "OK"), if markerContractFail then Color.RED else Color.GRAY);
AddLabel(yes, "RAW: SETUP " + (if longSetupPulseReady then "L" else if shortSetupPulseReady then "S" else "-") + " | CONT " + (if longContinuationPressure then "L" else if shortContinuationPressure then "S" else "-"), Color.GRAY);
