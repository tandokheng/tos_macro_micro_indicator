...
def longReadyEntry = longSetup and longEntryConfirm;
def shortReadyEntry = shortSetup and shortEntryConfirm;
...
AddLabel(
    ...
    "WHY: " +
        (if dashDir == 1 then
            (if !longTradeOK then (if structureBlocked then "STRUCT" else "RVOL")
            ...
            (if !shortTradeOK then (if structureBlocked then "STRUCT" else "RVOL")
