...
- Install `MacroMicro_Simplified_v0.5.33.ts` into Thinkorswim study `_dk_codex_macro_micro_v1` and confirm every screenshot shows `BUILD: v0.5.33 MIX ESCAPE`, numeric `DBG REV`, lower `DBG VIS`, `DBG BOTH`, `DBG CHOP` or `DBG MIX` in sideways regions, fewer alternating chop arrows, no default large `L/S` marker bubbles, and no default green/red score-probe arrows.

## Todo

- If v0.5.33 does not show `BUILD: v0.5.33 MIX ESCAPE`, remove duplicate/old `_dk_codex_macro_micro_v1` studies and re-add the pasted source once.
- Confirm old `SPAM L/S`, default compact `L/S` marker bubbles, and green/red score-probe arrows are gone; v0.5.33 should emphasize fewer magenta/cyan review arrows in mixed chop.
- If large `L/S` marker bubbles remain, check the saved `showSignalBubbles` setting in TOS because v0.5.33 gates those bubbles behind that existing toggle.
- If clear-trend arrows become too sparse in v0.5.33, first reduce `reviewConflictLookbackBars` from `13` or turn `reviewConflictFilterEnabled` off for one screenshot before loosening source gates.
- If `DBG BOTH` rises but `DBG MIX` stays low in sideways areas, the conflict is being detected but escaping; inspect fast-break/efficient-move escape conditions before changing the arrow renderer.
