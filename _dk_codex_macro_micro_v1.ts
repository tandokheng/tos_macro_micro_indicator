...
def reviewLongSetupSignal = showSetupReviewArrows and (setupReviewLongEdge or setupReviewLongRefresh);
def reviewShortSetupSignal = showSetupReviewArrows and (setupReviewShortEdge or setupReviewShortRefresh);
def visualLongSetupReady = reviewLongSetupSignal;
def visualShortSetupReady = reviewShortSetupSignal;
def reviewLongSignal = realLongEntry or reviewLongSetupSignal;
def reviewShortSignal = realShortEntry or reviewShortSetupSignal;
...
plot ReviewLongArrowV0524 =
    if showReviewSignalArrows and reviewLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0524 =
    if showReviewSignalArrows and reviewShortSignal then high + off else Double.NaN;
ReviewLongArrowV0524.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0524.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0524.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0524.AssignValueColor(Color.CYAN);
ReviewLongArrowV0524.SetLineWeight(5);
ReviewShortArrowV0524.SetLineWeight(5);

plot ScoreProbeLongArrowV0524 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0524 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0524.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0524.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0524.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0524.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0524.SetLineWeight(3);
ScoreProbeShortArrowV0524.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.24 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.24 DEBUG COUNTS", Color.BLACK);
