...
# Active build: v0.5.47 NQ REVIEW.
...
# - v0.5.47 side-locks 4/6 ready dots, adds one invalidation bubble, and lets PSAR-aligned TP2 plans become runners.
# - PSAR gradient is shown as review context; hard stops remain hard risk caps.
...
input reviewUsePSARRunner = yes;
input reviewPSARStrongSlopeATR = 0.08;
...
input showReviewInvalidationBubble = yes;
input showReviewRiskPointLabel = yes;
input showReviewPSARLabel = yes;
...
def setupReadyLockedDir =
    CompoundValue(
        1,
        if setupNeutralCount >= setupDotNeutralResetBars then 0
        else if setupReadyLockedDir[1] == 0 and setupLongReadyEdge and setupRecentShortReady[1] == 0 then 1
        else if setupReadyLockedDir[1] == 0 and setupShortReadyEdge and setupRecentLongReady[1] == 0 then -1
        else if setupReadyLockedDir[1] > 0 and setupShortTriggered then 0
        else if setupReadyLockedDir[1] < 0 and setupLongTriggered then 0
        else setupReadyLockedDir[1],
        0
    );
def setupComingLongDot = setupReadyLockedDir == 1 and setupReadyLockedDir[1] == 0;
def setupComingShortDot = setupReadyLockedDir == -1 and setupReadyLockedDir[1] == 0;
...
def reviewPSARSlopeATR = if reviewTargetStopATR > 0 then AbsValue(psar - psar[1]) / reviewTargetStopATR else 0;
def reviewPSARAligned =
    (reviewTargetStopDir == 1 and close > psar) or
    (reviewTargetStopDir == -1 and close < psar);
def reviewPSARSlopeAligned =
    (reviewTargetStopDir == 1 and psar > psar[1]) or
    (reviewTargetStopDir == -1 and psar < psar[1]);
def reviewPSARStrong = reviewPSARAligned and reviewPSARSlopeAligned and reviewPSARSlopeATR >= reviewPSARStrongSlopeATR;
def reviewPSARPlanAligned =
    CompoundValue(
        1,
        if reviewPlanStart then ((visibleLongSignal and close > psar) or (visibleShortSignal and close < psar))
        else reviewPSARPlanAligned[1],
        no
    );
...
def reviewTP2RunnerCandidate = reviewTP2HitCandidate and reviewUsePSARRunner and reviewPSARAligned;
def reviewOppositeScoreInvalid =
    reviewPlanWindowActive and
    reviewTargetStopAge > 0 and
    ((reviewTargetStopDir == 1 and shortScore >= triggerThreshold and shortScore > longScore) or
     (reviewTargetStopDir == -1 and longScore >= triggerThreshold and longScore > shortScore));
def reviewScoreInvalid =
    reviewPlanWindowActive and
    reviewTargetStopAge > 0 and
    ((reviewTargetStopDir == 1 and longScore < standbyThreshold) or
     (reviewTargetStopDir == -1 and shortScore < standbyThreshold));
def reviewPSARFlipInvalid =
    reviewPlanWindowActive and
    reviewTargetStopAge > 0 and
    reviewPSARPlanAligned and
    !reviewPSARAligned;
def reviewConditionInvalidCandidate =
    reviewPlanWindowActive and
    reviewTargetStopAge > 0 and
    !reviewStopHitCandidate and
    (reviewOppositeScoreInvalid or reviewScoreInvalid or reviewPSARFlipInvalid);
def reviewOutcomeState =
    CompoundValue(
        1,
        if reviewPlanStart then 0
        else if reviewOutcomeState[1] >= 0 and reviewStopHitCandidate then -1
        else if reviewOutcomeState[1] >= 0 and reviewOutcomeState[1] != 2 and reviewConditionInvalidCandidate then -2
        else if reviewOutcomeState[1] >= 0 and reviewTP2RunnerCandidate then 3
        else if reviewOutcomeState[1] >= 0 and reviewTP2HitCandidate then 2
...
def reviewTP2Hit = (reviewOutcomeState == 2 or reviewOutcomeState == 3) and reviewOutcomeState[1] != 2 and reviewOutcomeState[1] != 3;
def reviewStopHit = reviewOutcomeState == -1 and reviewOutcomeState[1] != -1;
def reviewInvalidated = reviewOutcomeState == -2 and reviewOutcomeState[1] != -2;
def reviewTargetHit = reviewTP2Hit;
def reviewRunnerActive = reviewOutcomeState == 3;
def reviewTargetStopActive = reviewTargetStopDir != 0 and reviewTargetStopAge <= Max(1, reviewTargetStopHoldBars) and reviewOutcomeState != -1 and reviewOutcomeState != -2 and reviewOutcomeState != 2;
...
def reviewInvalidationBubblePrice = if reviewTargetStopDir == 1 then high + off * 2 else low - off * 2;
def reviewInvalidationBubbleUp = reviewTargetStopDir == 1;
...
plot ReviewLongArrowV0547 =
    if showReviewSignalArrows and visibleLongSignal then low - reviewArrowOff else Double.NaN;
plot ReviewShortArrowV0547 =
    if showReviewSignalArrows and visibleShortSignal then high + reviewArrowOff else Double.NaN;
ReviewLongArrowV0547.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0547.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0547.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0547.AssignValueColor(Color.CYAN);
ReviewLongArrowV0547.SetLineWeight(5);
ReviewShortArrowV0547.SetLineWeight(5);

plot ScoreProbeLongArrowV0547 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0547 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0547.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0547.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0547.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0547.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0547.SetLineWeight(3);
ScoreProbeShortArrowV0547.SetLineWeight(3);

plot MarkerLongDotV0547 =
    if arrowMarkerLong then low - compactMarkerOff else Double.NaN;
MarkerLongDotV0547.SetPaintingStrategy(PaintingStrategy.POINTS);
MarkerLongDotV0547.SetLineWeight(5);
MarkerLongDotV0547.SetDefaultColor(Color.MAGENTA);

plot MarkerShortDotV0547 =
    if arrowMarkerShort then high + compactMarkerOff else Double.NaN;
MarkerShortDotV0547.SetPaintingStrategy(PaintingStrategy.POINTS);
MarkerShortDotV0547.SetLineWeight(5);
MarkerShortDotV0547.SetDefaultColor(Color.CYAN);
...
plot ReviewTP1LineV0547 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewTP1Price else Double.NaN;
ReviewTP1LineV0547.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewTP1LineV0547.SetStyle(Curve.SHORT_DASH);
ReviewTP1LineV0547.SetLineWeight(2);
ReviewTP1LineV0547.SetDefaultColor(GlobalColor("CautionAmber"));

plot ReviewTP2LineV0547 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewTP2Price else Double.NaN;
ReviewTP2LineV0547.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewTP2LineV0547.SetStyle(Curve.SHORT_DASH);
ReviewTP2LineV0547.SetLineWeight(2);
ReviewTP2LineV0547.SetDefaultColor(GlobalColor("CautionAmber"));

plot ReviewStopLineV0547 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewStopPrice else Double.NaN;
ReviewStopLineV0547.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewStopLineV0547.SetStyle(Curve.SHORT_DASH);
ReviewStopLineV0547.SetLineWeight(2);
ReviewStopLineV0547.SetDefaultColor(Color.RED);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.47 TEST", Color.CYAN, yes);
...
AddChartBubble(showReviewTargetStopBubbles and showReviewInvalidationBubble and reviewInvalidated, reviewInvalidationBubblePrice, "R-INV", Color.GRAY, reviewInvalidationBubbleUp);
...
AddLabel(yes, "BUILD: v0.5.47 NQ REVIEW", Color.BLACK);
AddLabel(showReviewRiskPointLabel and reviewTargetStopActive, "RISK PTS: " + AsText(Round(reviewRiskPoints, 2)) + " | TP1 " + AsText(Round(reviewTP1Points, 2)) + " | TP2 " + AsText(Round(reviewTP2Points, 2)) + (if reviewRunnerActive then " | RUN PSAR" else ""), Color.GRAY);
AddLabel(showReviewPSARLabel and reviewTargetStopActive, "PSAR: " + (if reviewPSARStrong then "STRONG" else if reviewPSARAligned then "OK" else if reviewPSARPlanAligned then "FLIP RISK" else "WATCH") + " " + AsText(Round(reviewPSARSlopeATR, 2)), if reviewPSARStrong then Color.GREEN else if reviewPSARAligned then Color.GRAY else GlobalColor("CautionAmber"));
