...
# Active build: v0.5.39 FOLLOW STRUCT.
...
# - DBG SET / DBG REV / DBG CHOP / DBG CONT / DBG FAST / DBG STRUCT / DBG BOTH / DBG ESC / DBG MIX / DBG PROBE labels separate setup, review, chop block, continuation block, fast-break block, structure/follow-through block, detected conflict, escaped conflict, blocked conflict, and probe cadence.
...
plot ReviewLongArrowV0539 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0539 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0539.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0539.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0539.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0539.AssignValueColor(Color.CYAN);
ReviewLongArrowV0539.SetLineWeight(5);
ReviewShortArrowV0539.SetLineWeight(5);

plot ScoreProbeLongArrowV0539 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0539 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0539.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0539.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0539.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0539.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0539.SetLineWeight(3);
ScoreProbeShortArrowV0539.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.39 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.39 FOLLOW STRUCT", Color.BLACK);
