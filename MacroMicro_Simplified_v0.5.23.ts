...
plot ReviewLongArrowV0523 =
    if showReviewSignalArrows and reviewLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0523 =
    if showReviewSignalArrows and reviewShortSignal then high + off else Double.NaN;
ReviewLongArrowV0523.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0523.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0523.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0523.AssignValueColor(Color.CYAN);
ReviewLongArrowV0523.SetLineWeight(5);
ReviewShortArrowV0523.SetLineWeight(5);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.23 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.23 REVIEW REFRESH", Color.BLACK);
