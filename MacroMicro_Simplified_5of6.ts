...
def noTradeRisk = mixedBias or chop;
def cautionRisk = !noTradeRisk and (extended or lowVolume or adx < chopADX);
def riskNormal = !noTradeRisk and !cautionRisk;

# ---------------- Trigger Conditions ----------------
def candleTR = TrueRange(high, close, low);
def avgTR20 = Average(candleTR, 20);
def bigCandle = candleTR > avgTR20 * 1.2;
def strongCloseLong = close >= high - candleTR * 0.25;
def strongCloseShort = close <= low + candleTR * 0.25;

def momentumLong = bigCandle and strongCloseLong and close > open;
def momentumShort = bigCandle and strongCloseShort and close < open;

def trendRejectionLong = low <= emaFast and close > emaFast and close > open;
def trendRejectionShort = high >= emaFast and close < emaFast and close < open;

def longTrigger = microCrossUp or momentumLong or trendRejectionLong;
def shortTrigger = microCrossDown or momentumShort or trendRejectionShort;

def longSetup = longScore >= triggerThreshold and longScore > shortScore and !noTradeRisk;
def shortSetup = shortScore >= triggerThreshold and shortScore > longScore and !noTradeRisk;
...
def signalMemoryExpired = barsSinceEntryEdge[1] > resetSameSideAfterBars;

def lastEntryDir =
    CompoundValue(
        1,
        if signalMemoryExpired then 0
        ...
        else if lastEntryDir[1] == 1 and shortBias then 0
        else if lastEntryDir[1] == -1 and longBias then 0
        else lastEntryDir[1],
        0
    );
...
def longStandby = longBias and !longTrigger and !noTradeRisk and activeDir != 1;
def shortStandby = shortBias and !shortTrigger and !noTradeRisk and activeDir != -1;
...
# ---------------- Bubbles + Alerts ----------------
AddChartBubble(showEntryBubbles and newLongEntry, low - off * 1.4, "LONG " + AsText(longScore) + "/6", Color.GREEN, no);
AddChartBubble(showEntryBubbles and newShortEntry, high + off * 1.4, "SHORT " + AsText(shortScore) + "/6", Color.RED, yes);

AddChartBubble(showExitBubbles and targetHit and activeDir[1] == 1, high + off, "PTARGET", Color.YELLOW, yes);
AddChartBubble(showExitBubbles and targetHit and activeDir[1] == -1, low - off, "PTARGET", Color.YELLOW, no);
AddChartBubble(showExitBubbles and stopHit and activeDir[1] == 1, low - off, "SLOSS", Color.RED, no);
AddChartBubble(showExitBubbles and stopHit and activeDir[1] == -1, high + off, "SLOSS", Color.RED, yes);
AddChartBubble(showExitBubbles and oppositeInvalidation and !targetHit and !stopHit, close, "EXIT", Color.ORANGE, activeDir[1] == -1);
...
AddLabel(
    dashOK,
    "RISK: " +
        (if riskNormal then "NORMAL"
         else if cautionRisk then "CAUTION"
         else "NO TRADE"),
    if riskNormal then Color.GREEN else if cautionRisk then Color.YELLOW else Color.RED
);
