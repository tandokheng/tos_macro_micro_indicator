...
- Keep the chart readable: one visible long marker, one visible short marker, standby dots, compact optional signal bubbles, optional target/stop bubbles, and review-only `R-TP` / `R-SL` overlays with arrows spaced outside the risk bubbles when judging arrow quality.
...
- Doing: Review/import `MacroMicro_Simplified_v0.5.42.ts`, a QQQ 5m review-risk build that keeps the proven arrow renderer, offsets arrows away from review TP/SL bubbles, and uses a review-only 2 ATR target so remaining arrows can be judged against less conservative follow-through.
