...
- Keep the chart readable: 4/6 setup-coming dots, next-candle 5/6 review arrows, compact optional signal bubbles, optional target/stop bubbles, and review-only `R-TP1` / `R-TP2` / `R-SL` overlays with arrows spaced outside the risk bubbles when judging arrow quality.
...
- Doing: Review/import `MacroMicro_Simplified_v0.5.43.ts`, a QQQ 5m entry-plan build where dots mean 4/6 setup-coming, arrows mean next-candle 5/6 trigger confirmation, and TP1/TP2/SL review overlays help judge whether signals have enough follow-through.
