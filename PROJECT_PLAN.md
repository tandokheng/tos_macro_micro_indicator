...
- Keep QQQ regular-session ETF/options validation separate from `/NQ` or `/MNQ` extended-hours futures validation.
...
- Doing: Review/import `MacroMicro_Simplified_v0.5.45.ts`, a QQQ 5m quiet-dot build where one 4/6 setup-coming dot prints per setup campaign, new dots require neutral cooldown or 5/6 trigger resolution, arrows mean next-candle 5/6 trigger confirmation, and TP1/TP2/SL review overlays help judge follow-through.
...
- Todo: Decide whether the next live visual validation should stay on QQQ regular session or move to `/NQ` / `/MNQ` as a separate futures track.
