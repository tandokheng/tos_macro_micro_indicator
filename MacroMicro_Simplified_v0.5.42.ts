...
# Active build: v0.5.42 RISK SPACE.
...
# - Review arrows use a risk-aware ATR offset so R-TP/R-SL bubbles do not cover the arrow body.
# - Review-only target math widens to a 2 ATR follow-through check while real trade PT/SL inputs remain unchanged.
...
def reviewLongCandidate = realLongEntryForReview or reviewLongSetupSignal;
def reviewShortCandidate = realShortEntryForReview or reviewShortSetupSignal;
...
def reviewTargetStopEntry =
    CompoundValue(
        1,
        if visibleLongSignal or visibleShortSignal then close else reviewTargetStopEntry[1],
        close
    );
def reviewTargetStopATR =
    CompoundValue(
        1,
        if visibleLongSignal or visibleShortSignal then atr else reviewTargetStopATR[1],
        atr
    );
...
def reviewTargetStopActive = reviewTargetStopDir != 0 and reviewTargetStopAge <= Max(1, reviewTargetStopHoldBars);
def reviewTargetPrice = if reviewTargetStopDir == 1 then reviewTargetStopEntry + reviewTargetStopATR * reviewTargetATRFactor else if reviewTargetStopDir == -1 then reviewTargetStopEntry - reviewTargetStopATR * reviewTargetATRFactor else Double.NaN;
def reviewStopPrice = if reviewTargetStopDir == 1 then reviewTargetStopEntry - reviewTargetStopATR * reviewStopATRFactor else if reviewTargetStopDir == -1 then reviewTargetStopEntry + reviewTargetStopATR * reviewStopATRFactor else Double.NaN;
def reviewTargetHitNow = reviewTargetStopActive and reviewTargetStopAge > 0 and ((reviewTargetStopDir == 1 and high >= reviewTargetPrice) or (reviewTargetStopDir == -1 and low <= reviewTargetPrice));
def reviewStopHitNow = reviewTargetStopActive and reviewTargetStopAge > 0 and ((reviewTargetStopDir == 1 and low <= reviewStopPrice) or (reviewTargetStopDir == -1 and high >= reviewStopPrice));
def reviewTargetHit = reviewTargetHitNow and !reviewTargetHitNow[1];
def reviewStopHit = reviewStopHitNow and !reviewStopHitNow[1];
...
def reviewArrowOff = Max(liveArrowOff, atr * (reviewStopATRFactor + reviewArrowRiskBufferATR));
...
plot ReviewLongArrowV0542 =
    if showReviewSignalArrows and visibleLongSignal then low - reviewArrowOff else Double.NaN;
plot ReviewShortArrowV0542 =
    if showReviewSignalArrows and visibleShortSignal then high + reviewArrowOff else Double.NaN;
ReviewLongArrowV0542.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0542.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0542.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0542.AssignValueColor(Color.CYAN);
ReviewLongArrowV0542.SetLineWeight(5);
ReviewShortArrowV0542.SetLineWeight(5);

plot ScoreProbeLongArrowV0542 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0542 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0542.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0542.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0542.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0542.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0542.SetLineWeight(3);
ScoreProbeShortArrowV0542.SetLineWeight(3);
...
plot ReviewTargetLineV0542 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewTargetPrice else Double.NaN;
ReviewTargetLineV0542.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewTargetLineV0542.SetStyle(Curve.SHORT_DASH);
ReviewTargetLineV0542.SetLineWeight(2);
ReviewTargetLineV0542.SetDefaultColor(GlobalColor("CautionAmber"));

plot ReviewStopLineV0542 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewStopPrice else Double.NaN;
ReviewStopLineV0542.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewStopLineV0542.SetStyle(Curve.SHORT_DASH);
ReviewStopLineV0542.SetLineWeight(2);
ReviewStopLineV0542.SetDefaultColor(Color.RED);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.42 TEST", Color.CYAN, yes);
...
AddChartBubble(showReviewTargetStopBubbles and visibleLongSignal, close + atr * reviewTargetATRFactor, "R-TP", GlobalColor("CautionAmber"), yes);
AddChartBubble(showReviewTargetStopBubbles and visibleLongSignal, close - atr * reviewStopATRFactor, "R-SL", Color.RED, no);
AddChartBubble(showReviewTargetStopBubbles and visibleShortSignal, close - atr * reviewTargetATRFactor, "R-TP", GlobalColor("CautionAmber"), no);
AddChartBubble(showReviewTargetStopBubbles and visibleShortSignal, close + atr * reviewStopATRFactor, "R-SL", Color.RED, yes);
AddChartBubble(showReviewTargetStopBubbles and reviewTargetHit, reviewTargetPrice, "R-TP HIT", GlobalColor("CautionAmber"), reviewTargetStopDir == 1);
AddChartBubble(showReviewTargetStopBubbles and reviewStopHit, reviewStopPrice, "R-SL HIT", Color.RED, reviewTargetStopDir == -1);
...
AddLabel(yes, "BUILD: v0.5.42 RISK SPACE", Color.BLACK);
