...
# Active build: v0.5.30 CHOP FILTER.
...
# - Setup-review arrows also need local trend efficiency and score dominance to avoid chop clusters.
...
def reviewChopBlockedLong = setupReviewLongQualityRaw and !reviewLongRegimeOK;
def reviewChopBlockedShort = setupReviewShortQualityRaw and !reviewShortRegimeOK;
...
plot ReviewLongArrowV0530 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0530 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0530.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0530.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0530.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0530.AssignValueColor(Color.CYAN);
ReviewLongArrowV0530.SetLineWeight(5);
ReviewShortArrowV0530.SetLineWeight(5);

plot ScoreProbeLongArrowV0530 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0530 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0530.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0530.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0530.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0530.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0530.SetLineWeight(3);
ScoreProbeShortArrowV0530.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.30 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.30 CHOP FILTER", Color.BLACK);
