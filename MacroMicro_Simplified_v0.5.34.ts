...
# - Mixed-direction review candidates use a wider 21-bar conflict horizon before fast-break or efficient VWAP/EMA-aligned escapes.
...
plot ReviewLongArrowV0534 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0534 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0534.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0534.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0534.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0534.AssignValueColor(Color.CYAN);
ReviewLongArrowV0534.SetLineWeight(5);
ReviewShortArrowV0534.SetLineWeight(5);

plot ScoreProbeLongArrowV0534 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0534 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0534.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0534.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0534.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0534.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0534.SetLineWeight(3);
ScoreProbeShortArrowV0534.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.34 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.34 WIDE MIX", Color.BLACK);
