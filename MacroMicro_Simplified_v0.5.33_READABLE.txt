...
# Active build: v0.5.33 MIX ESCAPE.
...
# - Mixed-direction review candidates are blocked unless a fast breakout or efficient VWAP/EMA-aligned move escapes.
# - DBG SET / DBG REV / DBG BOTH / DBG MIX / DBG PROBE labels separate setup, review, detected conflict, blocked conflict, and probe cadence.
...
def reviewLongConflictEscape = fastBreakoutConfirm or (reviewEfficientMove and reviewLongScoreDominant and close > emaSlow and close > vwapLine);
def reviewShortConflictEscape = fastBreakdownConfirm or (reviewEfficientMove and reviewShortScoreDominant and close < emaSlow and close < vwapLine);
...
plot ReviewLongArrowV0533 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0533 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0533.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0533.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0533.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0533.AssignValueColor(Color.CYAN);
ReviewLongArrowV0533.SetLineWeight(5);
ReviewShortArrowV0533.SetLineWeight(5);

plot ScoreProbeLongArrowV0533 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0533 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0533.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0533.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0533.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0533.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0533.SetLineWeight(3);
ScoreProbeShortArrowV0533.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.33 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.33 MIX ESCAPE", Color.BLACK);
