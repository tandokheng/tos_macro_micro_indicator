...
# Active build: v0.5.32 MIX FILTER.
...
# - Mixed-direction review candidates are blocked unless a breakout or continuation escape is present.
...
input reviewConflictLookbackBars = 13;
...
def reviewBothSidesRecent = Sum(if reviewLongCandidate then 1 else 0, reviewConflictBars) > 0 and Sum(if reviewShortCandidate then 1 else 0, reviewConflictBars) > 0;
def reviewLongConflictEscape = fastBreakoutConfirm or longContinuationPressure or (reviewEfficientMove and reviewLongScoreDominant and close > emaSlow and close > vwapLine);
def reviewShortConflictEscape = fastBreakdownConfirm or shortContinuationPressure or (reviewEfficientMove and reviewShortScoreDominant and close < emaSlow and close < vwapLine);
def reviewConflictBlockedLong = reviewConflictFilterEnabled and reviewBothSidesRecent and reviewLongCandidate and !reviewLongConflictEscape;
def reviewConflictBlockedShort = reviewConflictFilterEnabled and reviewBothSidesRecent and reviewShortCandidate and !reviewShortConflictEscape;
def reviewLongSignal = reviewLongCandidate and !reviewConflictBlockedLong;
def reviewShortSignal = reviewShortCandidate and !reviewConflictBlockedShort;
...
plot ReviewLongArrowV0532 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0532 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0532.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0532.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0532.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0532.AssignValueColor(Color.CYAN);
ReviewLongArrowV0532.SetLineWeight(5);
ReviewShortArrowV0532.SetLineWeight(5);

plot ScoreProbeLongArrowV0532 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0532 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0532.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0532.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0532.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0532.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0532.SetLineWeight(3);
ScoreProbeShortArrowV0532.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.32 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.32 MIX FILTER", Color.BLACK);
