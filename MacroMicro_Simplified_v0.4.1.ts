...
def debugForcedLongArrow = debugForceArrows and longScore >= triggerThreshold and longScore > shortScore and longTradeOK;
def debugForcedShortArrow = debugForceArrows and shortScore >= triggerThreshold and shortScore > longScore and shortTradeOK;
...
plot LongArrow = if newLongEntry or debugForcedLongArrow then low - arrowOff else Double.NaN;
LongArrow.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
LongArrow.SetLineWeight(5);
LongArrow.SetDefaultColor(Color.GREEN);
LongArrow.AssignValueColor(if debugForcedLongArrow and !newLongEntry then Color.MAGENTA else Color.GREEN);

plot ShortArrow = if newShortEntry or debugForcedShortArrow then high + arrowOff else Double.NaN;
ShortArrow.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ShortArrow.SetLineWeight(5);
ShortArrow.SetDefaultColor(Color.RED);
ShortArrow.AssignValueColor(if debugForcedShortArrow and !newShortEntry then Color.CYAN else Color.RED);
...
AddChartBubble(
    showSignalBubbles and debugForcedLongArrow and !newLongEntry,
    low - arrowOff * 1.4,
    "TEST L " +
        (if longScore >= 6 then "6" else if longScore >= 5 then "5" else if longScore >= 4 then "4" else if longScore >= 3 then "3" else if longScore >= 2 then "2" else if longScore >= 1 then "1" else "0") +
        "/6",
...
AddChartBubble(
    showSignalBubbles and debugForcedShortArrow and !newShortEntry,
    high + arrowOff * 1.4,
    "TEST S " +
        (if shortScore >= 6 then "6" else if shortScore >= 5 then "5" else if shortScore >= 4 then "4" else if shortScore >= 3 then "3" else if shortScore >= 2 then "2" else if shortScore >= 1 then "1" else "0") +
        "/6",
