...
# Active build: v0.5.26 CLEAN VISUALS.
...
input setupReviewRefreshBars = 13;
input reviewDebugLookbackBars = 200;
input showScoreProbeArrows = yes;
...
def setupReviewLongQuality = longEntryConfirm or lowVolumeLongMomentum or longContinuationPressure or longScore >= 6 or (close > emaFast and close > close[1]);
def setupReviewShortQuality = shortEntryConfirm or lowVolumeShortMomentum or shortContinuationPressure or shortScore >= 6 or (close < emaFast and close < close[1]);
...
def scoreProbeLongSignal = showScoreProbeArrows and longSetupPulseReady and setupReviewCadenceSlot;
def scoreProbeShortSignal = showScoreProbeArrows and shortSetupPulseReady and setupReviewCadenceSlot;
...
def realLongEntryForReview = realLongEntryReviewFlag == 1;
def realShortEntryForReview = realShortEntryReviewFlag == 1;
def reviewLongSetupSignalRaw = showSetupReviewArrows and (setupReviewLongEdge or setupReviewLongRefresh);
def reviewShortSetupSignalRaw = showSetupReviewArrows and (setupReviewShortEdge or setupReviewShortRefresh);
...
def visibleLongSignal = reviewLongSignal;
def visibleShortSignal = reviewShortSignal;
...
def scoreProbeLongRecentCount = Sum(if scoreProbeLongSignal then 1 else 0, reviewDebugLookbackBars);
def scoreProbeShortRecentCount = Sum(if scoreProbeShortSignal then 1 else 0, reviewDebugLookbackBars);
...
def dashboardTriggerLong = fastFlipLongArrow or dashboardContLong or realLongEntry or reviewLongSetupSignal;
def dashboardTriggerShort = fastFlipShortArrow or dashboardContShort or realShortEntry or reviewShortSetupSignal;
...
plot ReviewLongArrowV0526 =
    if showReviewSignalArrows and reviewLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0526 =
    if showReviewSignalArrows and reviewShortSignal then high + off else Double.NaN;
ReviewLongArrowV0526.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0526.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0526.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0526.AssignValueColor(Color.CYAN);
ReviewLongArrowV0526.SetLineWeight(5);
ReviewShortArrowV0526.SetLineWeight(5);

plot ScoreProbeLongArrowV0526 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0526 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0526.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0526.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0526.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0526.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0526.SetLineWeight(3);
ScoreProbeShortArrowV0526.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.26 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.26 CLEAN VISUALS", Color.BLACK);
