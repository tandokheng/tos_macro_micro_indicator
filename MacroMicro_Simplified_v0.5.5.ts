...
def realLongEntry = newLongEntry or practicalLongArrow or fastFlipLongArrow or longContinuationArrow or setupPulseLongArrow;
def realShortEntry = newShortEntry or practicalShortArrow or fastFlipShortArrow or shortContinuationArrow or setupPulseShortArrow;
...
plot SignalLongV055Arrow = if realLongEntry then low - signalArrowOff else Double.NaN;
SignalLongV055Arrow.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
SignalLongV055Arrow.SetLineWeight(5);
SignalLongV055Arrow.SetDefaultColor(Color.GREEN);
SignalLongV055Arrow.AssignValueColor(if debugForcedLongArrow and !realLongEntry then Color.MAGENTA else Color.GREEN);

plot SignalShortV055Arrow = if realShortEntry then high + signalArrowOff else Double.NaN;
SignalShortV055Arrow.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
SignalShortV055Arrow.SetLineWeight(5);
SignalShortV055Arrow.SetDefaultColor(Color.RED);
SignalShortV055Arrow.AssignValueColor(if debugForcedShortArrow and !realShortEntry then Color.CYAN else Color.RED);
    ...
    "NEXT: " +
        (if dashDir == 1 then
            (if dashScore < standbyThreshold or (dashScore < triggerThreshold and !allowCautionFourOfSix) then "SCORE"
             ...
             else if longSetupPulseReady then "SETUP PULSE"
            ...
            (if dashScore < standbyThreshold or (dashScore < triggerThreshold and !allowCautionFourOfSix) then "SCORE"
             ...
             else if shortSetupPulseReady then "SETUP PULSE"
