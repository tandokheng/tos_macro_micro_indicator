...
# Active build: v0.5.41 REVIEW RISK.
...
def reviewTargetPrice = if reviewTargetStopDir == 1 then reviewTargetStopEntry + reviewTargetStopATR * targetATRFactor else if reviewTargetStopDir == -1 then reviewTargetStopEntry - reviewTargetStopATR * targetATRFactor else Double.NaN;
def reviewStopPrice = if reviewTargetStopDir == 1 then reviewTargetStopEntry - reviewTargetStopATR * stopATRFactor else if reviewTargetStopDir == -1 then reviewTargetStopEntry + reviewTargetStopATR * stopATRFactor else Double.NaN;
...
plot ReviewLongArrowV0541 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0541 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0541.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0541.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0541.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0541.AssignValueColor(Color.CYAN);
ReviewLongArrowV0541.SetLineWeight(5);
ReviewShortArrowV0541.SetLineWeight(5);

plot ScoreProbeLongArrowV0541 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0541 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0541.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0541.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0541.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0541.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0541.SetLineWeight(3);
ScoreProbeShortArrowV0541.SetLineWeight(3);
...
plot ReviewTargetLineV0541 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewTargetPrice else Double.NaN;
ReviewTargetLineV0541.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewTargetLineV0541.SetStyle(Curve.SHORT_DASH);
ReviewTargetLineV0541.SetLineWeight(2);
ReviewTargetLineV0541.SetDefaultColor(GlobalColor("CautionAmber"));

plot ReviewStopLineV0541 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewStopPrice else Double.NaN;
ReviewStopLineV0541.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewStopLineV0541.SetStyle(Curve.SHORT_DASH);
ReviewStopLineV0541.SetLineWeight(2);
ReviewStopLineV0541.SetDefaultColor(Color.RED);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.41 TEST", Color.CYAN, yes);
...
AddChartBubble(showReviewTargetStopBubbles and visibleLongSignal, close + atr * targetATRFactor, "R-TP", GlobalColor("CautionAmber"), yes);
AddChartBubble(showReviewTargetStopBubbles and visibleLongSignal, close - atr * stopATRFactor, "R-SL", Color.RED, no);
AddChartBubble(showReviewTargetStopBubbles and visibleShortSignal, close - atr * targetATRFactor, "R-TP", GlobalColor("CautionAmber"), no);
AddChartBubble(showReviewTargetStopBubbles and visibleShortSignal, close + atr * stopATRFactor, "R-SL", Color.RED, yes);
...
AddLabel(yes, "BUILD: v0.5.41 REVIEW RISK", Color.BLACK);
