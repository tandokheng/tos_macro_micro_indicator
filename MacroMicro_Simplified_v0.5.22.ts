...
def reviewLongSetupSignal = showSetupReviewArrows and setupReviewLongEdge;
def reviewShortSetupSignal = showSetupReviewArrows and setupReviewShortEdge;
...
plot ReviewLongArrowV0522 =
    if showReviewSignalArrows and reviewLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0522 =
    if showReviewSignalArrows and reviewShortSignal then high + off else Double.NaN;
ReviewLongArrowV0522.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0522.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0522.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0522.AssignValueColor(Color.CYAN);
ReviewLongArrowV0522.SetLineWeight(5);
ReviewShortArrowV0522.SetLineWeight(5);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.22 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.22 BALANCED ARROWS", Color.BLACK);
