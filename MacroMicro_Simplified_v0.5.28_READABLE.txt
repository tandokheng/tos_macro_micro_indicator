...
# Active build: v0.5.28 TIGHT REVIEW.
...
def setupReviewLongQuality = longEntryConfirm or lowVolumeLongMomentum or longContinuationPressure or (longScore >= 6 and setupReviewLongDirectional);
def setupReviewShortQuality = shortEntryConfirm or lowVolumeShortMomentum or shortContinuationPressure or (shortScore >= 6 and setupReviewShortDirectional);
...
plot ReviewLongArrowV0528 =
    if showReviewSignalArrows and reviewLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0528 =
    if showReviewSignalArrows and reviewShortSignal then high + off else Double.NaN;
ReviewLongArrowV0528.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0528.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0528.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0528.AssignValueColor(Color.CYAN);
ReviewLongArrowV0528.SetLineWeight(5);
ReviewShortArrowV0528.SetLineWeight(5);

plot ScoreProbeLongArrowV0528 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0528 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0528.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0528.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0528.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0528.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0528.SetLineWeight(3);
ScoreProbeShortArrowV0528.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.28 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.28 TIGHT REVIEW", Color.BLACK);
