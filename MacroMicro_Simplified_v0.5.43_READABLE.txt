...
# Active build: v0.5.43 ENTRY PLAN.
...
# - Compact marker dots now mean setup is coming: 4/6 get-ready state, not the 5/6 trigger arrow.
...
def setupComingLong = longTradeOK and longScore == standbyThreshold and longScore > shortScore;
def setupComingShort = shortTradeOK and shortScore == standbyThreshold and shortScore > longScore;
def setupComingLongDot = setupComingLong and !setupComingLong[1];
def setupComingShortDot = setupComingShort and !setupComingShort[1];
...
plot ReviewLongArrowV0543 =
    if showReviewSignalArrows and visibleLongSignal then low - reviewArrowOff else Double.NaN;
plot ReviewShortArrowV0543 =
    if showReviewSignalArrows and visibleShortSignal then high + reviewArrowOff else Double.NaN;
ReviewLongArrowV0543.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0543.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0543.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0543.AssignValueColor(Color.CYAN);
ReviewLongArrowV0543.SetLineWeight(5);
ReviewShortArrowV0543.SetLineWeight(5);

plot ScoreProbeLongArrowV0543 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0543 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0543.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0543.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0543.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0543.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0543.SetLineWeight(3);
ScoreProbeShortArrowV0543.SetLineWeight(3);

plot MarkerLongDotV0543 =
    if arrowMarkerLong then low - compactMarkerOff else Double.NaN;
MarkerLongDotV0543.SetPaintingStrategy(PaintingStrategy.POINTS);
MarkerLongDotV0543.SetLineWeight(5);
MarkerLongDotV0543.SetDefaultColor(Color.MAGENTA);

plot MarkerShortDotV0543 =
    if arrowMarkerShort then high + compactMarkerOff else Double.NaN;
MarkerShortDotV0543.SetPaintingStrategy(PaintingStrategy.POINTS);
MarkerShortDotV0543.SetLineWeight(5);
MarkerShortDotV0543.SetDefaultColor(Color.CYAN);
...
plot ReviewTP1LineV0543 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewTP1Price else Double.NaN;
ReviewTP1LineV0543.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewTP1LineV0543.SetStyle(Curve.SHORT_DASH);
ReviewTP1LineV0543.SetLineWeight(2);
ReviewTP1LineV0543.SetDefaultColor(GlobalColor("CautionAmber"));

plot ReviewTP2LineV0543 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewTP2Price else Double.NaN;
ReviewTP2LineV0543.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewTP2LineV0543.SetStyle(Curve.SHORT_DASH);
ReviewTP2LineV0543.SetLineWeight(2);
ReviewTP2LineV0543.SetDefaultColor(GlobalColor("CautionAmber"));

plot ReviewStopLineV0543 =
    if showReviewTargetStopLines and reviewTargetStopActive then reviewStopPrice else Double.NaN;
ReviewStopLineV0543.SetPaintingStrategy(PaintingStrategy.HORIZONTAL);
ReviewStopLineV0543.SetStyle(Curve.SHORT_DASH);
ReviewStopLineV0543.SetLineWeight(2);
ReviewStopLineV0543.SetDefaultColor(Color.RED);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.43 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.43 ENTRY PLAN", Color.BLACK);
