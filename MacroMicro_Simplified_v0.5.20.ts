...
def visualLongSetupReady = rawSetupLongMarker;
def visualShortSetupReady = rawSetupShortMarker;
...
def visibleLongSignal = realLongEntry or continuationAnchorLongArrow or visualLongSetupReady;
def visibleShortSignal = realShortEntry or continuationAnchorShortArrow or visualShortSetupReady;
...
def dashboardTriggerLong = fastFlipLongArrow or dashboardContLong or visualLongSetupReady or realLongEntry;
def dashboardTriggerShort = fastFlipShortArrow or dashboardContShort or visualShortSetupReady or realShortEntry;
...
def longSignalCaution = longScore < triggerThreshold or longTradeCaution or fastFlipLongArrow or longContinuationArrow or visualLongSetupReady;
def shortSignalCaution = shortScore < triggerThreshold or shortTradeCaution or fastFlipShortArrow or shortContinuationArrow or visualShortSetupReady;
...
plot RawSetupLongArrowV0520 =
    if showRawSetupArrows and rawSetupBubbleLong then low - off else Double.NaN;
plot RawSetupShortArrowV0520 =
    if showRawSetupArrows and rawSetupBubbleShort then high + off else Double.NaN;
RawSetupLongArrowV0520.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
RawSetupShortArrowV0520.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
RawSetupLongArrowV0520.AssignValueColor(Color.MAGENTA);
RawSetupShortArrowV0520.AssignValueColor(Color.CYAN);
RawSetupLongArrowV0520.SetLineWeight(5);
RawSetupShortArrowV0520.SetLineWeight(5);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.20 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.20 CLEAN ARROWS", Color.BLACK);
...
AddLabel(
    ...
    "TRIGGER: " +
        (if longDangerEdge then "DANGER LONG"
         ...
         else if visualLongSetupReady then "SETUP LONG"
         else if visualShortSetupReady then "SETUP SHORT"
    ...
    else if visualLongSetupReady then Color.GREEN
    else if visualShortSetupReady then Color.RED
...
AddLabel(
    ...
    "CAUTION BY: " +
        (if fastFlipShortSetup then "BREAKDOWN"
         ...
         else if visualShortSetupReady or visualLongSetupReady then "SETUP"
    ...
    "NEXT: " +
        (if dashDir == 1 then
            (if dashScore < standbyThreshold or (dashScore < triggerThreshold and !allowCautionFourOfSix) then "SCORE"
              ...
              else if visualLongSetupReady then "MARK READY"
            ...
            (if dashScore < standbyThreshold or (dashScore < triggerThreshold and !allowCautionFourOfSix) then "SCORE"
              ...
              else if visualShortSetupReady then "MARK READY"
