...
- File: `MacroMicro_Simplified_v0.5.22.ts`
...
## What v0.5.22 Is Testing

- `BUILD: v0.5.22 BALANCED ARROWS` is always visible through a static `AddLabel(yes, ...)`; if absent, TOS is not running this pasted build or study labels are hidden.
...
- v0.5.22 keeps raw setup labels for diagnosis but uses only first 5/6 setup edges or 6/6 upgrades for default setup-review arrows.
- v0.5.22 uses fresh dedicated `ReviewLongArrowV0522` / `ReviewShortArrowV0522` plots for `realLongEntry` / `realShortEntry` plus balanced setup-review edges.
...
- A cyan `v0.5.22 TEST` bubble and `BuildProofDotV0513` point remain on the last loaded bar to prove the chart-bubble/point drawing paths are active independent of signal logic.
...
- Confirm the pasted source header shows `# Version: v0.5.22`.
- Confirm the chart labels show `BUILD: v0.5.22 BALANCED ARROWS`, `RAW SETUP`, `RAW CONT`, and `CONTRACT: OK` or `FAIL TRIGGER/MARKER`.
- Confirm old `SPAM L` / `SPAM S` bubbles are gone.
- Confirm direct `RAW L` / `RAW S` proof bubbles are hidden by default.
- Confirm magenta/cyan arrows are present again, but only on setup-review edges, 6/6 upgrades, and real entries rather than repeated raw cadence.
