...
- File: `MacroMicro_Simplified_v0.5.18.ts`
...
## What v0.5.18 Is Testing

- `BUILD: v0.5.18 RAW THIN` is always visible through a static `AddLabel(yes, ...)`; if absent, TOS is not running this pasted build or study labels are hidden.
...
- A cyan `v0.5.18 TEST` bubble and `BuildProofDotV0513` point remain on the last loaded bar to prove the chart-bubble/point drawing paths are active independent of signal logic.
...
- Confirm the pasted source header shows `# Version: v0.5.18`.
- Confirm the chart labels show `BUILD: v0.5.18 RAW THIN`, `RAW SETUP`, `RAW CONT`, and `CONTRACT: OK` or `FAIL TRIGGER/MARKER`.
