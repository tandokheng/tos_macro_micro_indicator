...
## What v0.5.24 Is Testing

- `BUILD: v0.5.24 DEBUG COUNTS` is always visible through a static `AddLabel(yes, ...)`; if absent, TOS is not running this pasted build or study labels are hidden.
...
- v0.5.24 uses fresh dedicated `ReviewLongArrowV0524` / `ReviewShortArrowV0524` plots for `realLongEntry` / `realShortEntry`, setup edges, and slow sustained setup-review refreshes.
- v0.5.24 adds visual-only `ScoreProbeLongArrowV0524` / `ScoreProbeShortArrowV0524` arrows that fire from 5/6 setup cadence before the quality gate, but only when the normal review arrow is silent.
...
- A cyan `v0.5.24 TEST` bubble and `BuildProofDotV0513` point remain on the last loaded bar to prove the chart-bubble/point drawing paths are active independent of signal logic.
...
- If magenta/cyan review arrows are absent, read the debug counts: nonzero `DBG SET` plus zero `DBG REV` means the review gate is too strict; nonzero `DBG PROBE` should also show green/red probe arrows on some historical bars.
