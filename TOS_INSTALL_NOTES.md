...
- File: `MacroMicro_Simplified_v0.5.9.ts`
...
## What v0.5.9 Is Testing

- Real arrows use `realLongEntry` / `realShortEntry`, which combine the strict entry-state path with practical setup-edge arrows.
- Continuation-anchor arrows use `visibleLongSignal` / `visibleShortSignal`, so `NEXT: CONTINUE S/L` can paint arrows without resetting entry price, ATR, target, or stop tracking.
...
- Sustained 5/6 setups can produce `SETUP S/L` caution arrows every `setupPulseBars` bars; v0.5.9 keeps this at 5 bars as the middle ground.
...
- Confirm the header shows `# Version: v0.5.9` in the pasted source.
