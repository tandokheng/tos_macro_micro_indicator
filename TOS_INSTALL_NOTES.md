...
- File: `MacroMicro_Simplified_v0.5.21.ts`
...
## What v0.5.21 Is Testing

- `BUILD: v0.5.21 QUIET ARROWS` is always visible through a static `AddLabel(yes, ...)`; if absent, TOS is not running this pasted build or study labels are hidden.
...
- v0.5.21 keeps raw setup labels for diagnosis but removes raw setup from default arrows, markers, and dashboard triggers.
- v0.5.21 uses fresh dedicated `ReviewLongArrowV0521` / `ReviewShortArrowV0521` plots for `realLongEntry` / `realShortEntry` only.
...
- A cyan `v0.5.21 TEST` bubble and `BuildProofDotV0513` point remain on the last loaded bar to prove the chart-bubble/point drawing paths are active independent of signal logic.
...
- Confirm the pasted source header shows `# Version: v0.5.21`.
- Confirm the chart labels show `BUILD: v0.5.21 QUIET ARROWS`, `RAW SETUP`, `RAW CONT`, and `CONTRACT: OK` or `FAIL TRIGGER/MARKER`.
- Confirm old `SPAM L` / `SPAM S` bubbles are gone.
- Confirm direct `RAW L` / `RAW S` proof bubbles are hidden by default.
- Confirm magenta/cyan arrows are much less frequent because raw setup alone no longer plots default arrows.
