...
- File: `MacroMicro_Simplified_v0.5.5.ts`
...
- Primary arrows use fresh plot names `SignalLongV055Arrow` and `SignalShortV055Arrow` to avoid inherited hidden/custom TOS settings from earlier plot names.
...
- If there are still no arrows while `TRIGGER` shows `SETUP LONG` or `SETUP SHORT`, reset the study's plot visibility/style settings or import under a fresh study name.

## Current Manual Test Flow

- Paste `_dk_codex_macro_micro_v1.ts` into the Thinkorswim study `_dk_codex_macro_micro_v1`.
- Confirm the header shows `# Version: v0.5.5` in the pasted source.
