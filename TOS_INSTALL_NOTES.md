...
- File: `MacroMicro_Simplified_v0.5.38.ts`
- Import-ready file: `_dk_codex_macro_micro_v1.ts`
- Side-panel readable copy: `MacroMicro_Simplified_v0.5.38_READABLE.txt`
- Thinkorswim study to replace: `_dk_codex_macro_micro_v1`
- Primary chart for review: QQQ 5-minute, 20-day view

## What v0.5.38 Is Testing

- `BUILD: v0.5.38 STRUCT GUARD` is always visible through a static `AddLabel(yes, ...)`; if absent, TOS is not running this pasted build or study labels are hidden.
...
- v0.5.34 widens `reviewConflictLookbackBars` from 13 to 21 because v0.5.33 screenshots showed `DBG BOTH L/S` was alive but too low relative to the visible chop cycle.
- v0.5.35 keeps the 21-bar conflict lookback but tightens mixed-conflict escape to `minConflictEscapeTrendEfficiency = 0.45` and `minConflictEscapeScoreSeparation = 3`, because v0.5.34 screenshots showed conflict was detected but some candidates still escaped.
- v0.5.36 fixes the next chop leakage path: continuation pressure can no longer bypass the review chop gate unless it breaks a wider 5-bar structure window and has 2-point side dominance.
- v0.5.37 fixes the next chop leakage path: raw fast-break confirmation can no longer bypass the review chop gate or escape mixed-direction conflict unless it also passes a stronger review-only 5-bar structure break and 1.60 range proof.
- v0.5.38 fixes the next chop leakage path: normal efficient-score review arrows can no longer paint unless they also break 5-bar local structure.
- `DBG BOTH L/S` counts candidates where both directions were detected within the mixed lookback. `DBG ESC L/S` counts detected-conflict candidates that escaped the mixed filter. `DBG MIX L/S` counts candidates actually blocked by that mixed-direction filter. `DBG CONT L/S` counts continuation-pressure candidates blocked because they did not make the wider structure break. `DBG FAST L/S` counts raw fast-break candidates blocked because they did not make the stronger review fast break. `DBG STRUCT L/S` counts score/efficiency candidates blocked because they did not break local structure.
...
- A cyan `v0.5.38 TEST` bubble and `BuildProofDotV0513` point remain on the last loaded bar to prove the chart-bubble/point drawing paths are active independent of signal logic.
- Real trade entries and PT/SL tracking still use `realLongEntry` / `realShortEntry`; v0.5.38 only filters the review-arrow display path.
...
- If the `.ts` file does not open in Codex side panel, use the clipboard handoff or open `MacroMicro_Simplified_v0.5.38_READABLE.txt`; it is verified to match the `.ts` source exactly.
- Confirm the pasted source header shows `# Version: v0.5.38`.
- Confirm the chart labels show `BUILD: v0.5.38 STRUCT GUARD`, `RAW SETUP`, `RAW CONT`, `DBG SET L200/S200`, `DBG REV L200/S200`, `DBG VIS L/S`, `DBG CHOP L/S`, `DBG CONT L/S`, `DBG FAST L/S`, `DBG STRUCT L/S`, `DBG BOTH L/S`, `DBG ESC L/S`, `DBG MIX L/S`, `DBG PROBE L/S`, and `CONTRACT: OK` or `FAIL TRIGGER/MARKER`.
...
- Confirm `DBG VIS L/S` is lower than `DBG REV L200/S200` and that `DBG CHOP L/S`, `DBG CONT L/S`, `DBG FAST L/S`, `DBG STRUCT L/S`, `DBG BOTH L/S`, `DBG ESC L/S`, or `DBG MIX L/S` explains sideways regions where v0.5.37 was still noisy.
- If large `L/S` bubbles still appear, check whether `showSignalBubbles` was saved on in TOS; v0.5.38 gates those compact marker bubbles behind that existing toggle.
- If v0.5.38 becomes too quiet in clean trends, first lower `reviewTrendBreakLookbackBars` from `5` to `4` for one screenshot before changing the working arrow renderer.
