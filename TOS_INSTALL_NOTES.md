...
- File: `MacroMicro_Simplified_v0.5.23.ts`
...
## What v0.5.23 Is Testing

- `BUILD: v0.5.23 REVIEW REFRESH` is always visible through a static `AddLabel(yes, ...)`; if absent, TOS is not running this pasted build or study labels are hidden.
...
- v0.5.23 uses fresh dedicated `ReviewLongArrowV0523` / `ReviewShortArrowV0523` plots for `realLongEntry` / `realShortEntry`, setup edges, and slow sustained setup-review refreshes.
...
- A cyan `v0.5.23 TEST` bubble and `BuildProofDotV0513` point remain on the last loaded bar to prove the chart-bubble/point drawing paths are active independent of signal logic.
...
- Confirm the pasted source header shows `# Version: v0.5.23`.
- Confirm the chart labels show `BUILD: v0.5.23 REVIEW REFRESH`, `RAW SETUP`, `RAW CONT`, and `CONTRACT: OK` or `FAIL TRIGGER/MARKER`.
- Confirm old `SPAM L` / `SPAM S` bubbles are gone.
- Confirm direct `RAW L` / `RAW S` proof bubbles are hidden by default.
- Confirm magenta/cyan arrows are present again on past candles, but less dense than v0.5.20 because sustained setup refreshes use a slow quality-gated cadence.
