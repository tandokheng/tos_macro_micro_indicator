...
# Active build: v0.5.40 FLIP GUARD.
...
# - Final review arrows also remember recent opposite raw pressure so alternating chop flips stay blocked unless a stronger breakout or confirmed trend structure appears.
...
input minPressureConflictEscapeTrendEfficiency = 0.55;
input minPressureConflictEscapeScoreSeparation = 4;
...
def reviewLongConflictEscape = reviewFastBreakoutOK or (reviewConflictEfficientMove and reviewLongConflictScoreDominant and close > emaSlow and close > vwapLine);
def reviewShortConflictEscape = reviewFastBreakdownOK or (reviewConflictEfficientMove and reviewShortConflictScoreDominant and close < emaSlow and close < vwapLine);
...
def reviewLongPressureConflictEscape = reviewFastBreakoutOK or (reviewPressureConflictEfficientMove and reviewLongPressureConflictScoreDominant and reviewLongTrendBreakOK and close > vwapLine);
def reviewShortPressureConflictEscape = reviewFastBreakdownOK or (reviewPressureConflictEfficientMove and reviewShortPressureConflictScoreDominant and reviewShortTrendBreakOK and close < vwapLine);
...
plot ReviewLongArrowV0540 =
    if showReviewSignalArrows and visibleLongSignal then low - off else Double.NaN;
plot ReviewShortArrowV0540 =
    if showReviewSignalArrows and visibleShortSignal then high + off else Double.NaN;
ReviewLongArrowV0540.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ReviewShortArrowV0540.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ReviewLongArrowV0540.AssignValueColor(Color.MAGENTA);
ReviewShortArrowV0540.AssignValueColor(Color.CYAN);
ReviewLongArrowV0540.SetLineWeight(5);
ReviewShortArrowV0540.SetLineWeight(5);

plot ScoreProbeLongArrowV0540 =
    if scoreProbeLongSignal and !reviewLongSignal then low - off * 2 else Double.NaN;
plot ScoreProbeShortArrowV0540 =
    if scoreProbeShortSignal and !reviewShortSignal then high + off * 2 else Double.NaN;
ScoreProbeLongArrowV0540.SetPaintingStrategy(PaintingStrategy.ARROW_UP);
ScoreProbeShortArrowV0540.SetPaintingStrategy(PaintingStrategy.ARROW_DOWN);
ScoreProbeLongArrowV0540.AssignValueColor(Color.GREEN);
ScoreProbeShortArrowV0540.AssignValueColor(Color.RED);
ScoreProbeLongArrowV0540.SetLineWeight(3);
ScoreProbeShortArrowV0540.SetLineWeight(3);
...
AddChartBubble(lastVisibleBar, high + liveArrowOff * 1.3, "v0.5.40 TEST", Color.CYAN, yes);
...
AddLabel(yes, "BUILD: v0.5.40 FLIP GUARD", Color.BLACK);
