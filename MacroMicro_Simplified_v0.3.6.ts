...
def anyEntryEdge = longEntryEdge or shortEntryEdge or longReadyEdge or shortReadyEdge;
...
def longEntryPulse = (rawLongEntry or longReadyEntry) and (longEntryEdge or longReadyEdge or cooldownJustCleared or longTradeJustUnblocked);
def shortEntryPulse = (rawShortEntry or shortReadyEntry) and (shortEntryEdge or shortReadyEdge or cooldownJustCleared or shortTradeJustUnblocked);

def lastEntryDir =
    CompoundValue(
        1,
        if lastEntryDir[1] == 1 and (shortBias or longScore < triggerThreshold) then 0
        else if lastEntryDir[1] == -1 and (longBias or shortScore < triggerThreshold) then 0
        else if longEntryPulse and cooldownOK and lastEntryDir[1] != 1 then 1
        else if shortEntryPulse and cooldownOK and lastEntryDir[1] != -1 then -1
        else lastEntryDir[1],
        0
    );
...
def newLongEntry = longEntryPulse and cooldownOK and lastEntryDir[1] != 1;
def newShortEntry = shortEntryPulse and cooldownOK and lastEntryDir[1] != -1;
...
def continuationLongSignal = longEntryPulse and cooldownOK and lastEntryDir[1] == 1 and !trendResetLong;
def continuationShortSignal = shortEntryPulse and cooldownOK and lastEntryDir[1] == -1 and !trendResetShort;
    ...
    "WHY: " +
        (if dashDir == 1 then
            (if dashScore < standbyThreshold or (dashScore < triggerThreshold and !allowCautionFourOfSix) then "SCORE"
             ...
             else if lastEntryDir[1] == 1 then "SAME"
